...
## Installation
Using the install script, the following will be installed under the `~/.ayushman-vllm-metal` directory (the default).
- vllm-metal plugin
- vllm core
- Related libraries

If you run `source ~/.ayushman-vllm-metal/bin/activate`, the `vllm` CLI becomes available and you can access the vLLM right away.
